	...
	<section>
		<article>
			...
			<c:if test="${not empty warning}">
				<p
					style="color: orange; background-color: #fff3cd; padding: 10px; border: 1px solid orange; border-radius: 5px;">
						...
						<tbody>
							<c:forEach var='item' items="${cartItems}">
								<tr>
									...
									<td><c:out value="${item.price}" /> &euro;</td>
									<td><c:out value="${item.quantityInCart}" /></td>
									<td><c:choose>
											<c:when test="${item.availableStock < item.quantityInCart}">
												<span style="color: red; font-weight: bold;"> <c:out
														value="${item.availableStock}" /> (WARNING)
												</span>
											</c:when>
											<c:otherwise>
												<c:out value="${item.availableStock}" />
											</c:otherwise>
										</c:choose></td>
									...
									<td>
										<form action="removeFromCart" method="post"
											...
											<input type="hidden" name="${_csrf.parameterName}"
												...
												name="bookId" value="${item.id}" /> <input type="submit"
												value="Remove" />
										</form>
									</td>
								</tr>
							...
							<tr>
								...
								<td colspan="2"><strong><c:out value="${total}" />
										&euro;</strong></td>
							</tr>
						</tbody>
					</table>
